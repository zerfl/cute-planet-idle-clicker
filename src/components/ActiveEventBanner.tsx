...
export const ActiveEventBanner: React.FC<ActiveEventBannerProps> = React.memo(({
  ...
  const getEventStyle = () => {
    if (!activeEvent) return "bg-[#110e28]/85 border-cosmic-accent/45 text-cosmic-accent-muted shadow-inner";
    ...
    if (id.includes("solar") || id.includes("tempest") || id.includes("radiation") || id.includes("core") || id.includes("puls") || id.includes("dust") || id.includes("storm") || id.includes("eruption")) {
    ...
    if (id.includes("nebula") || id.includes("galaxy") || id.includes("void") || id.includes("magnetic") || id.includes("matrix") || id.includes("rift") || id.includes("supernova") || id.includes("hyper")) {
  ...
  return (
    <section className={`w-full max-w-2xl p-5 rounded-3xl border-3 flex flex-col gap-4.5 transition-all duration-500 shadow-lg relative overflow-hidden ${getEventStyle()}`}>
      {activeEvent ? (
            ...
            <div className="flex items-start gap-3.5 w-full text-left">
              ...
              <div className="flex-grow min-w-0">
                ...
                <p className="text-[10.5px] font-semibold opacity-90 leading-relaxed mt-1 max-w-xl text-slate-250">
                  {activeEventDetails?.description || "Ein geheimnisvolles Flackern zieht an deinem Planeten vorbei..."}
                </p>
              </div>
            </div>
            ...
            <div className="bg-black/40 p-3.5 rounded-2xl border border-white/10 flex flex-col gap-3">
                    ...
                    <span className="text-emerald-400 flex items-center gap-1">
                      ...
                      <strong className="underline uppercase font-black tracking-wide">
                        {activeEventDecision === "ignorieren"
                          ? "🧘 Ignorieren"
                          : activeEventDetails?.options.find((o) => o.id === activeEventDecision)?.name || "Ausgewählt"}
                      </strong>
                    </span>
                  ) : (
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {activeEventDetails?.options.map((opt) => {
                  ...
                  let optIcon = "✨";
                  if (opt.name.includes("Sammeln") || opt.name.includes("Ernte") || opt.name.includes("Kapsel") || opt.name.includes("Energie")) optIcon = "🌾";
                  else if (opt.name.includes("Forschung") || opt.name.includes("Analys") || opt.name.includes("Daten") || opt.name.includes("Stud") || opt.name.includes("Verlauf")) optIcon = "🔬";
                  else if (opt.name.includes("Zerleg") || opt.name.includes("Kristall") || opt.name.includes("Staub") || opt.name.includes("Ader")) optIcon = "💎";
                  else if (opt.name.includes("Sterne") || opt.name.includes("Glan") || opt.name.includes("Stellar") || opt.name.includes("Konstell")) optIcon = "⭐";
                  else if (opt.name.includes("Mond") || opt.name.includes("Trabant") || opt.name.includes("Kollaps") || opt.name.includes("Gravit")) optIcon = "🌙";
                  else if (opt.name.includes("Opfer") || opt.name.includes("Absorption") || opt.name.includes("Kollision") || opt.name.includes("Sturm")) optIcon = "💥";
                ...
                >
                  ...
                  <p className="text-[9.5px] opacity-80 leading-tight text-slate-300">
                    Ereignis träge abwarten: Erhöht Dauer um 60s, gewährt aber einen kleinen passiven LPS-Bonus.
                  </p>
                </button>
              </div>
              {activeEventDecision && (
