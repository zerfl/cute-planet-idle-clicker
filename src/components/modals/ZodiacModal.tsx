    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col items-center text-center">
          <div
            className={`p-1.5 rounded-full bg-gradient-to-tr ${isNight ? "from-purple-800 to-indigo-950" : "from-amber-100 to-yellow-50"} mb-4`}
          >
            <span className="text-4xl filter drop-shadow-md select-none">{zodiac.emoji}</span>
          </div>

          <h4
            className={`font-sans font-black text-xl tracking-tight leading-none ${
              isNight
          ...
          <div className="mt-5 mb-5 relative group">
            <div
              className={`absolute -inset-1 rounded-3.5xl bg-gradient-to-r ${isNight ? "from-purple-500 to-cosmic-accent" : "from-amber-400 to-amber-500"} opacity-35 blur-md`}
            ></div>
            <div
          ...
          >
            <span
              className={`block text-[10px] uppercase font-black tracking-widest ${isNight ? "text-purple-300" : "text-amber-800"} mb-1`}
            >
              Kosmische Aura
            </span>
            <p className="font-sans font-bold text-xs leading-relaxed">{zodiac.description}</p>
            ...
            <div className="flex items-center gap-1.5 font-sans font-black text-xs">
              <Sparkles
                className={`w-4 h-4 ${isNight ? "text-cosmic-accent animate-pulse" : "text-amber-600"}`}
              />
...
ZodiacModal.displayName = "ZodiacModal";
